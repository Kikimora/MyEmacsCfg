...
(defsubst js2-consume-type-specificier ()
  (when (= js2-COLON (js2-peek-token))
    (js2-consume-token)
    (js2-peek-token)
    (js2-consume-token)
    (when (eq (js2-peek-token) js2-LB)
          (js2-consume-token)
	  (js2-peek-token)
	  (js2-consume-token))))

(defsubst js2-consume-reserved ()
  (when (= js2-RESERVED (js2-peek-token))
    (js2-consume-token)))
...
(defun js2-parse-toplevel (root stop-token)
  (let (n)
    (while (and (/= (setq tt (js2-peek-token)) js2-EOF) (or (null stop-token) (/= tt stop-token)))
      (setq n nil)
      (cond
       ((eq tt js2-RESERVED)
	(js2-consume-token))
       ((eq tt js2-CLASS)
	(setq n (js2-parse-class root)))
       ((eq tt js2-FUNCTION)
	(js2-consume-token)
...
(defun js2-do-parse (&optional stop-token)
...
(defun js2-parse-class (root)
  (let (tt result n)
    (js2-must-match js2-CLASS "msg.no.class.kwd")
    (js2-must-match js2-NAME "msg.no.class.name")
    (when (eq (setq tt (js2-peek-token)) js2-EXTENDS)
      (js2-consume-token)
      (js2-must-match js2-NAME "msg.no.class.name"))
    (js2-must-match js2-LC "msg.no.brace.body")
    (message "aaa")
    (setq result (js2-parse-toplevel root js2-RC))
    (js2-consume-token)
    result))
...
Returns the parsed `js2-var-decl-node' expression node."
  (let* ((result (make-js2-var-decl-node :decl-type decl-type
    ...
    (while continue
      ...
      ;; Parse typed var expressions like var r : Rect
      (js2-consume-type-specificier)
